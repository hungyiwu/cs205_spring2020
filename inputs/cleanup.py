...
phonopy_inputs_dir = '/n/holyscratch01/cs205/group4/example-relax/'
...
for dir in os.listdir(os.getcwd()+vasp_dir_new):
    if not re.match('config',dir):
        ...
        copyfile("CONTCAR", phonopy_inputs_dir+dir+"/POSCAR-unit")
        v.relax_off(phonopy_inputs_dir+dir+"/POSCAR-unit") # turn off selective dynamics
        ...
        p.phonopy_config_writer(band=phonopy_inputs_dir+dir+'/band.conf',mesh=phonopy_inputs_dir+dir+'/mesh.conf')
        os.chdir(master_dir)
